...
function ClientDetailView() {
  ...
  // Single ask card — centered as the hero when empty, pinned as a floating
    ...
    <AppShell title={client?.name ?? "Client"}>
      <div className="canvas-warm relative flex h-full flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <div
            className={cn(
              "mx-auto flex min-h-full max-w-3xl flex-col px-5 pt-10",
              empty ? "justify-center pb-6" : "pb-4"
            )}
          >
            {loading ? (
              ...
              <>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {t.sources.map((s, j) => (
                                  <span
                                    ...
                                    className="inline-flex items-center gap-1.5 rounded-full bg-muted px-2 py-0.5 text-[11px] text-muted-foreground"
                                  >
                ...
                {/* empty state: ask card sits centered as the hero */}
                {empty && <div className="mt-2">{askCard}</div>}
        ...
        {/* chat started: floating composer pinned to the bottom */}
        {!loading && !empty && (
          <div className="shrink-0 bg-background/70 backdrop-blur">
            <div className="mx-auto max-w-3xl px-5 pb-5 pt-2">{askCard}</div>
          </div>
        )}
      </div>
    </AppShell>
